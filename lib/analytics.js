...
/**
 * Calculate fasting hours since last meal
 * @param {string} lastMealDate - Date of last meal (YYYY-MM-DD)
 * @param {string} lastMealTime - Time of last meal (HH:MM:SS or HH:MM)
 * @returns {number|null} Hours since last meal, or null if no data
 */
export function calculateFastingHours(lastMealDate, lastMealTime) {
  if (!lastMealDate || !lastMealTime) return null
  
  // Combine date and time into a full datetime
  // Handle both HH:MM:SS and HH:MM formats
  const timeParts = lastMealTime.split(':')
  const hours = parseInt(timeParts[0], 10)
  const minutes = parseInt(timeParts[1], 10)
  const seconds = timeParts[2] ? parseInt(timeParts[2], 10) : 0
  
  const lastMealDateTime = new Date(lastMealDate + 'T00:00:00')
  lastMealDateTime.setHours(hours, minutes, seconds)
  
  const now = new Date()
  const diffMs = now - lastMealDateTime
  
  // Don't return negative values (future meals)
