...
export async function searchBrandedFoods(query, pageSize = 25) {
  try {
    ...
    // Silently fail - Open Food Facts provides backup coverage
    // DEMO_KEY has rate limits; get a free key at fdc.nal.usda.gov/api-key-signup.html
...
function transformFood(food) {
  ...
  if (food.foodNutrients) {
    food.foodNutrients.forEach(nutrient => {
      ...
      if (id === 1079) nutrients.fiber = value
    })
  }
